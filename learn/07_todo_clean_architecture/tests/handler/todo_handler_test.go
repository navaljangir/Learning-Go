...
func setupTodoTestRouter(todoService service.TodoService) *gin.Engine {
	...
	router.POST("/api/v1/todos", authMiddleware, h.Create)
	router.GET("/api/v1/todos", authMiddleware, h.List)
	router.GET("/api/v1/todos/:id", authMiddleware, h.GetByID)
	router.PUT("/api/v1/todos/:id", authMiddleware, h.Update)
	router.PATCH("/api/v1/todos/:id/toggle", authMiddleware, h.ToggleComplete)
	router.DELETE("/api/v1/todos/:id", authMiddleware, h.Delete)
